...
export const TimelineScreen: React.FC = () => {
  return (
    <div className="min-h-screen pt-24 pb-32 px-4 max-w-4xl mx-auto relative">
      
      {/* Header */}
      <div className="fixed top-0 left-0 right-0 z-40 p-4 pointer-events-none flex justify-between items-start">
         <div className="pointer-events-auto bg-paper-white/90 backdrop-blur px-6 py-2 rounded shadow-float transform -rotate-1 border border-primary/10 mt-2 ml-2">
            <h1 className="font-hand text-3xl text-primary">Our Journey</h1>
         </div>
      </div>
      ...
      <div className="flex justify-center mb-24 relative z-10">
        <div className="bg-paper-white px-6 py-6 shadow-float rotate-2 text-center border-2 border-dashed border-pencil/30 rounded-sm">
      ...
      {TIMELINE_DATA.map((item, index) => (
        <div key={item.id} className={`relative flex flex-col md:flex-row items-center justify-between mb-32 group ${index % 2 !== 0 ? 'md:flex-row-reverse' : ''}`}>
          ...
          {/* Text/Note Section */}
          <div className={`w-full md:w-[45%] flex justify-center ${index % 2 === 0 ? 'md:justify-start md:pl-12' : 'md:justify-end md:pr-12'} mt-8 md:mt-0`}>
             <div className="bg-sticky p-6 shadow-float transform rotate-1 w-64 relative">
                {/* Pin */}
      ...
      <div className="flex flex-col items-center mt-20 relative z-10">
         <div className="bg-transparent border-2 border-primary/20 rounded-full px-8 py-3 text-ink font-display font-medium hover:bg-primary/5 transition-colors cursor-pointer">
            Continue the Story...
         </div>
      </div>
    </div>
